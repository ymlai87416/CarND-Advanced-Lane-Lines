...
def image_pipeline(img_input, left_fit, right_fit, is_debug):
    ...
    sxbinary = abs_sobel_thresh(img_undist_enhanced, 'x', 20,
                                100)  # [(scaled_sobel >= thresh_min) & (scaled_sobel <= thresh_max)] = 1
    s_binary = hls_select(img_undist_enhanced,
                          (170, 255))  # [(s_channel >= s_thresh_min) & (s_channel <= s_thresh_max)] = 1
    ...
    img_birdview_lane = plot_left_and_right_lane(img_birdview_clr, img_birdview_bin, left_fit, right_fit,
    ...
    if(is_debug):
        ...
        img_hist = Image.fromarray(
            cv2.resize(img_hist, (img_input.shape[1] // 2, img_input.shape[0] // 6), interpolation=cv2.INTER_CUBIC))
        ...
        debug_im.paste(
            Image.fromarray(cv2.resize(img_undist_enhanced, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_CUBIC)),
            (0, img_input.shape[0]))
        debug_im.paste(Image.fromarray(
            cv2.resize(img_birdview_bin * 255, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_CUBIC)).convert('RGB'),
                       (img_input.shape[1] // 2, img_input.shape[0]))
        debug_im.paste(Image.fromarray(cv2.resize(color_mask_clr, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_CUBIC)),
                       (img_input.shape[1], 0))
        debug_im.paste(Image.fromarray(cv2.resize(color_binary, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_CUBIC)),
                       (img_input.shape[1], img_input.shape[0] // 2))
        debug_im.paste(Image.fromarray(cv2.resize(img_birdview_lane, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_CUBIC)),
...
def process_image(image):
    ...
    try:
        result, left_fit, right_fit = image_pipeline(image, left_fit, right_fit, False)
...
def process_image_debug(image):
    ...
    try:
        result, left_fit, right_fit = image_pipeline(image, left_fit, right_fit, True)
    except Exception as ex:
        mpimg.imsave("error_images/error.jpg", image)
        traceback.print_exc()
        return image
...
if __name__ == '__main__':
    ...
    if(is_debug.lower() == 'true'):
        ...
        white_clip = clip1.fl_image(process_image_debug)  # NOTE: this function expects color images!!
    else:
